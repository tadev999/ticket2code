# Agent Stages — Integration Test Cases Generation

## Stages

### Stage 0 — Select output language (mandatory)
...
- Record selection as `Selected output language` and reuse it for all interactions and test report content.
...
- Classify test scope (unit, integration, end-to-end).
...
- Proposed test categories and environment setup
...
Save to: docs/test/integration/<TICKET-ID>_integration_tests_<predev|postdev>_<YYYYMMDDHHmm>.md

### Stage 5 — Categorize test cases
- Define non-overlapping test categories.
- Common categories: UI/UX, Business Logic, Data Persistence, API Integration, Error Handling, Performance, Security.
- Map each AC to one or more categories.
- Ensure exhaustive coverage (all test cases belong to a category).
...
- Define trigger/action for each step

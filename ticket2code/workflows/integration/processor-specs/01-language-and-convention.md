# Language and Convention — Integration Test Report

## Document Language and Style

### Language
- Language must come from explicit Stage 0 selection (`Selected output language`).
- Do not infer report language from ticket/AC language or user message language.
- Use `Selected output language` consistently for all conversation and report content.
...
- `Post-Dev` report should include evidence references when available.
...
### Test Case Names
Format: `TC-[CATEGORY]-[NNN]: [Scenario description]`

Examples:
- `TC-UI-001: User enters valid amount`
- `TC-BL-002: System enforces daily limit`
- `TC-API-003: Payment API timeout triggers retry`
...
### Categories
Use standard category names (from 03-test-categorization.md):
- `UI/UX`
- `Business Logic`
- `Data Persistence`
- `API Integration`
